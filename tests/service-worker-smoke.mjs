...
const baseUrl = "https://beta.frecka.app/releases/0.11.12-test/site/";
...
const cacheNames = new Set([
  ...
  "frecka-app-shell-0.11.10-betapreview001-1",
  "frecka-app-shell-0.11.11-betapreview002-1",
...
assert.ok(addedUrls.includes(`${baseUrl}styles.css?v=betapreview003-1`));
assert.ok(addedUrls.includes(`${baseUrl}vendor/jszip-v3.10.1.min.js?v=betapreview003-1`));
assert.ok(addedUrls.includes(`${baseUrl}js/export-package.js?v=betapreview003-1`));
assert.ok(addedUrls.includes(`${baseUrl}js/license-runtime.js?v=betapreview003-1`));
assert.ok(addedUrls.includes(`${baseUrl}js/pwa-update.js?v=betapreview003-1`));
assert.ok(addedUrls.includes(`${baseUrl}js/app.js?v=betapreview003-1`));
assert.ok(addedUrls.includes(`${baseUrl}js/backup.js?v=betapreview003-1`));
...
assert.equal(new URL(manifest.start_url, baseUrl).pathname, "/releases/0.11.12-test/site/index.html");
assert.equal(new URL(manifest.scope, baseUrl).pathname, "/releases/0.11.12-test/site/");
...
const currentCache = [...cacheNames].find(name => name === "frecka-app-shell-0.11.12-betapreview003-1");
assert.ok(currentCache, "Der versionsgebundene Cache wurde nicht angelegt.");
assert.ok(cacheNames.has("frecka-app-shell-0.11.10-betapreview001-1"), "Installation darf einen historischen Cache nicht vor bewusster Aktivierung entfernen.");
assert.ok(cacheNames.has("frecka-app-shell-0.11.11-betapreview002-1"), "Installation darf den unmittelbaren Vorgängercache nicht vor bewusster Aktivierung entfernen.");
...
assert.deepEqual(deletedCaches, ["frecka-app-shell-0.9.0-old", "frecka-app-shell-0.11.10-betapreview001-1", "frecka-app-shell-0.11.11-betapreview002-1"]);
...
const cachedAsset = { source: "cache", url: `${baseUrl}styles.css?v=betapreview003-1` };
