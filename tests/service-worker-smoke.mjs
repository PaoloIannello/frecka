...
const baseUrl = "https://beta.frecka.app/releases/0.10.2-test/site/";
...
assert.equal(skipWaitingCalls, 1, "Die einmalige SERVICEWORKER-002-Legacy-Brücke wurde nicht ausgeführt.");
...
assert.ok(addedUrls.includes(`${baseUrl}styles.css?v=serviceworker002-2`));
assert.ok(addedUrls.includes(`${baseUrl}vendor/jszip-v3.10.1.min.js?v=serviceworker002-2`));
assert.ok(addedUrls.includes(`${baseUrl}js/export-package.js?v=serviceworker002-2`));
assert.ok(addedUrls.includes(`${baseUrl}js/pwa-update.js?v=serviceworker002-2`));
assert.ok(addedUrls.includes(`${baseUrl}js/app.js?v=serviceworker002-2`));
...
assert.equal(new URL(manifest.start_url, baseUrl).pathname, "/releases/0.10.2-test/site/index.html");
assert.equal(new URL(manifest.scope, baseUrl).pathname, "/releases/0.10.2-test/site/");
...
assert.doesNotMatch(source, /clients\.claim\s*\(/, "Die Legacy-Brücke darf laufende Clients nicht übernehmen.");
assert.doesNotMatch(source, /indexedDB|localStorage|sessionStorage/, "Der Worker darf keine Geschäftsdaten berühren.");

const currentCache = [...cacheNames].find(name => name === "frecka-app-shell-0.10.2-serviceworker002-2");
...
assert.equal(skipWaitingCalls, 2, "Die bewusste Aktivierungsnachricht wurde nicht verarbeitet.");
...
const cachedAsset = { source: "cache", url: `${baseUrl}styles.css?v=serviceworker002-2` };
...
console.log("Service-Worker-Smoke-Test: PASS (Legacy-Brücke, Nachricht, App-Shell, Unterpfad, Offline-Fallback, Cache-Isolation)");
