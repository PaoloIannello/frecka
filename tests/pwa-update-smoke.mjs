...
assert.match(indexSource, /<title>FRECKA – PODOLOGY-006<\/title>/);
assert.match(appSource, /Aktualisierung nicht abgeschlossen/);
assert.match(appSource, /Erneut versuchen/);
assert.match(indexSource, /js\/pwa-update\.js\?v=podology006-1/);
...
assert.match(dataSource, /version:\s*"0\.11\.9"/);
assert.match(dataSource, /build:\s*"PODOLOGY-006"/);
