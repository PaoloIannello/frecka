...
assert.match(indexSource, /<title>FRECKA – BACKUP-006<\/title>/);
assert.match(appSource, /Aktualisierung nicht abgeschlossen/);
assert.match(appSource, /Erneut versuchen/);
assert.match(indexSource, /js\/pwa-update\.js\?v=backup006-1/);
...
assert.match(dataSource, /version:\s*"0\.11\.4"/);
assert.match(dataSource, /build:\s*"BACKUP-006"/);
