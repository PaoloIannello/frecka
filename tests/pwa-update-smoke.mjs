...
assert.match(indexSource, /<title>FRECKA – BETA-PREVIEW-001<\/title>/);
assert.match(appSource, /Aktualisierung nicht abgeschlossen/);
assert.match(appSource, /Erneut versuchen/);
assert.match(indexSource, /js\/pwa-update\.js\?v=betapreview001-1/);
...
assert.match(dataSource, /version:\s*"0\.11\.10"/);
assert.match(dataSource, /build:\s*"BETA-PREVIEW-001"/);
