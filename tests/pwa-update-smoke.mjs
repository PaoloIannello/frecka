...
assert.match(indexSource, /<title>FRECKA – PODOLOGY-004<\/title>/);
assert.match(appSource, /Aktualisierung nicht abgeschlossen/);
assert.match(appSource, /Erneut versuchen/);
assert.match(indexSource, /js\/pwa-update\.js\?v=podology004-1/);
...
assert.match(dataSource, /version:\s*"0\.11\.7"/);
assert.match(dataSource, /build:\s*"PODOLOGY-004"/);
