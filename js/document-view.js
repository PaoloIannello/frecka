(() => {
  ...
  function renderReceipt(model, options = {}) {
    ...
    return `<article class="receipt-paper ${isVoucherSale ? "is-voucher-sale" : ""}">
      ...
      ${model.prescriptionDate || model.customerCareAdvice ? `<div class="receipt-paper-customer-supplements">${model.prescriptionDate ? `<p><strong>Rezept vom:</strong> ${escapeHtml(model.prescriptionDate)}</p>` : ""}${model.customerCareAdvice ? `<p><strong>Pflegehinweis:</strong> ${escapeHtml(model.customerCareAdvice)}</p>` : ""}</div>` : ""}
