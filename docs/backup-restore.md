...
**Stand:** MULTI-COMPANY-004B auf Basis MULTI-COMPANY-002/003/004, BACKUP-006, PODOLOGY-006, BRANDING-002, TSE-002, SETTINGS-002, USER-001 und PERSISTENCE-007
...
PODOLOGY-002 ergänzt keine zweite Sicherungsstruktur: unveränderliche Rezeptzuordnungen liegen innerhalb der bereits gesicherten Ursprungsbelege. Die Snapshotvalidierung prüft ihre Rezept-, Kunden- und Geschäftsbereichsreferenzen sowie die aus Belegen und Vollstornos abgeleitete Nutzung. Alte Belege ohne Zuordnung bleiben gültig. PODOLOGY-004 darf nach Restore im lokalen Kundendokument ausschließlich das gesicherte Rezeptdatum aus dieser historischen Zuordnung anzeigen. Reguläre Exporte, Steuerberater-PDFs, QR und Public Viewer projizieren den Zuordnungssnapshot weiterhin nicht.
...
MULTI-COMPANY-002/003/004 liegen vollständig innerhalb der vorhandenen Storestruktur. Die Produktoberfläche kann mehrere vollständig referenzierte Profile mit eindeutigen IDs und Kürzeln verwalten; `activeCompanyId` wird mitgesichert und muss nach Restore auf ein vorhandenes Profil zeigen. `businessAreas` und `serviceLocations` sowie Belege, Gutscheine, Rezepte und Behandlungsdatensätze tragen ihre direkte `companyId`. Kunden, Benutzer, Logo-Asset-Register, Backup-Erinnerung, Behandlungsvorlagen, eingebettete Positionen und Gutscheinhistorien erhalten keine `companyId`. Die gerätelokale `licenseRuntime` bleibt ausgeschlossen; deshalb macht Restore ein zusätzliches Profil nie eigenständig produktiv. Das äußere Backupformat bleibt Version 1.

MULTI-COMPANY-004B behandelt `companies[].betaProductiveTest` ausdrücklich als installationslokalen, buildgebundenen Testzustand und nicht als Fachdaten- oder Lizenzberechtigung. `exportTenantSnapshot()` entfernt ihn vor Validierung, Verschlüsselung und Export. Restore schreibt deshalb keine Beta-Testfreigabe und aktiviert kein Zusatzprofil; auf dem Zielgerät gilt wieder `activation_required`, bis dort in einem ausdrücklich erlaubten Beta-Build erneut lokal freigegeben wird. Profile, Nummernkreise und bereits erzeugte Testgeschäftsvorgänge bleiben regulärer Bestandteil der Sicherung. Das äußere Backupformat bleibt Version 1.
...
LICENSE-005 führt die ab Schema 9 unter `stores.settings.companies[].license` liegende portable, nicht autoritative Referenz der Formatversion 2 je Profil fort. Alle portablen Profilreferenzen werden verschlüsselt mitgesichert und atomar wiederhergestellt. Der getrennte Store `licenseRuntime`, Geräte-ID, private und öffentliche CryptoKeys, Thumbprint, Lizenz-Token, Validierungs- und Zeitanker sowie gecachte Entitlements gehören ausdrücklich nicht zum Tenant-Snapshot und können durch Restore weder importiert noch ersetzt werden.

TSE-002 liegt als `companies[].tseSettings` je Profil ebenfalls innerhalb von `stores.settings`. Anbieter sowie deaktivierter Einrichtungs- und Verbindungsstatus werden dadurch verschlüsselt mitgesichert und atomar wiederhergestellt. Zugangsdaten, Tokens, Schlüssel und TSE-Transaktionen existieren in diesem Modell nicht.
...
- je Unternehmensprofil genau eine vollständige portable Lizenzreferenz Version 2 mit derselben `localTenantId`, profilweise eindeutiger opaker Lizenz-ID, festem Produkt `frecka.core`/Hauptversion 1 und konsistenter optionaler Serververknüpfung;
...
`tests/persistence-smoke.html` prüft ohne zusätzliches Testframework die gesamte bisherige Persistenz sowie BACKUP-001/002/003/004/005/006, HARDEN-001, EXPORT-001/003, PERSISTENCE-007/008/010, SETTINGS-001/002, TSE-002, LICENSE-005, MULTI-COMPANY-003/004/004B und QR-001. BACKUP-006 ergänzt historische Profile für 0.9.x/0.10.x sowie Zustände vor USER-001, LICENSE-001, SETTINGS-001/002, BRANDING-001/002, BACKUP-004 und TSE-002 einschließlich Kombination und PERSISTENCE-010. MULTI-COMPANY-003 ergänzt den exakten Backup-/Restore-Roundtrip und die anschließende Fortsetzung aller Profil-/Typ-/Jahreszähler, ohne alte Belegnummern zu verändern. MULTI-COMPANY-004 prüft zusätzlich mehrere Profile, den aktiven Kontext, die erhaltene Produktsperre, globale Kunden und unveränderte Nummernkontexte nach Restore. MULTI-COMPANY-004B weist zusätzlich nach, dass die lokale Testfreigabe weder Snapshot noch Restore noch Export erreicht und im Nicht-Beta-Build unwirksam ist. LICENSE-005 ergänzt Schema 5→6, portable V2-Migration, CryptoKey-Reload und den Nachweis, dass Runtime, Schlüssel und Token weder verschlüsselt gesichert noch restauriert werden. Jeder freigegebene Profilpfad durchläuft Startnormalisierung, Settingspersistenz, Snapshotvalidierung, AES-GCM, Restore, erneute Vollvalidierung und Idempotenz. Ein eigener Negativfall beweist `fail closed` und unveränderte Eingabedaten. Die Diagnoseprüfungen suchen gezielt nach Namen, Beträgen, IDs, Nummern und Referenzen und dürfen ausschließlich sichere Kategorien ausgeben.
