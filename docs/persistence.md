...
- `data.company`: optionale Geschäftsbezeichnung, verpflichtende rechtliche Person, optionaler Ansprechpartner, Unternehmensanschrift, Kontakt- und Steuerangaben, Website, aktive Unternehmenslogo-Referenz, eigener Änderungszeitpunkt und `useAsServiceLocation`;
- `data.logoAssets`: unveränderliches, versioniertes Register aller aktuellen und historisch referenzierten PNG-/JPEG-Logoversionen;
...
- `data.receiptSettings`: aktive profilinterne Nummernkreisprojektion, Belegtexte, Währung und Sprache;
...
- `data.license`: die portable, nicht autoritative Lizenzreferenz der Formatversion 2 des aktiven Profils; Gerätebindung und Schlüssel gehören ausschließlich in `licenseRuntime`;
- `data.tseSettings`: optionale, standardmäßig deaktivierte TSE-Vorbereitung für `fiskaly SIGN DE`, ohne Zugangsdaten oder Anbieterkommunikation;
...
4. Der fachlich leere Standardkatalog wird einmal in das aktuelle Laufzeitformat überführt und als sichere Fallbackbasis erfasst. Branchenvorlagen bleiben davon getrennt und werden nur nach ausdrücklicher Auswahl importiert.
...
9. Fehlt ein Datensatz, bleiben für den jeweiligen Bereich die neutralen technischen beziehungsweise fachlich leeren Standardwerte aktiv. Es existiert kein Demo-Dokument, das eine produktive Nummer verbrauchen könnte.
...
MULTI-COMPANY-004 lädt diese Laufzeitobjekte aus dem per `activeCompanyId` gewählten Profil. Beim Speichern werden nur dieses Profil sowie dessen Geschäftsbereiche, Leistungsorte und Vorlagen aktualisiert; fremde Profile und Zuordnungen bleiben erhalten. Neue Profile werden im selben Settings-Datensatz mit opaken IDs, explizitem sicherem Belegkürzel, eigenen Defaults und Nummernkreisen angelegt. Ein Zusatzprofil wird lokal als `activation_required` behandelt und kann bis zum späteren Lizenz-Multi-Binding nicht produktiv schreiben. Schema und Storestruktur bleiben unverändert.

MULTI-COMPANY-004C ergänzt darauf ausschließlich den schnellen Headerwechsel. Die UI verwendet für Unternehmen denselben persistierenden `activeCompanyId`-Wechsel wie die Einstellungen und für den Geschäftsbereich weiterhin den vorhandenen Laufzeitzustand. Angeboten werden nur aktive Bereiche des gewählten Profils. Overlayzustand, Fokus und geöffnete Auswahl werden nicht gespeichert; Backup-, Restore- und Exportformate bleiben unverändert.

MULTI-COMPANY-004B behält Schema 9 und alle Stores unverändert. Ein Zusatzprofil kann in den ausdrücklich erlaubten Builds `BETA-PREVIEW-002` und `BETA-PREVIEW-003` den lokalen Profilzustand `betaProductiveTest` Version 1 erhalten (`enabled`, `updatedAt`). Der zentrale Status bleibt `activation_required`; nur die kombinierte Prüfung aus gespeichertem Profilzustand und erlaubter kanonischer Buildkennung öffnet denselben bestehenden Write-Guard. Danach gelten ohne Sonderpfad alle atomaren Nummern-, Beleg-, Gutschein-, Korrektur- und Podologieprüfungen. Profil 1 verwendet den Zustand nicht. Alle anderen Builds ignorieren ihn. Die portable Snapshot-Projektion entfernt das Feld vor Backup, Restore, Diagnose und Export; dadurch bleibt es installationslokaler Testzustand und keine zweite Lizenzstruktur. Details: [ADR-0009](adr/ADR-0009-temporaere-beta-testfreigabe-fuer-zusatzprofile.md).
...
- Behandlungsdokumentationsformat-Version: `1`
...
PODOLOGY-003 hebt das Schema von 7 auf 8 und ergänzt ausschließlich den mandantenbezogenen Store `treatmentRecords`. Die Migration legt für die aktuelle Instanz und jeden vorhandenen Settings-Mandanten einen leeren Datensatz an; die sieben bisher vorhandenen Stores einschließlich `licenseRuntime` bleiben bytegleich. Ein Behandlungsdatensatz referenziert genau einen normalen Ursprungsbeleg, Kunden, Geschäftsbereich und aktiven Benutzer sowie optional das zugeordnete Rezept. Er enthält unveränderliche Kunden-, Geschäftsbereichs-, Benutzer- und optionale Rezept-Snapshots. Pro Beleg ist höchstens ein Datensatz zulässig.

Der bestehende Receipt-Writer liest und schreibt Settings/Nummernstand, Belege, Rezepte und Behandlungsdokumentation in derselben Readwrite-Transaktion. Das gilt auch für einen normalen Beleg mit Gutscheinzahlung. Nur wenn interne Dokumentation oder Kundenpflegehinweis nach dem Trimmen Inhalt besitzen, wird ein Datensatz erzeugt; ein Gutscheinverkaufsbeleg erzeugt niemals einen. Fehler, veraltete Referenzen, doppelte Belegzuordnungen oder ein beschädigter Store brechen die gesamte Transaktion einschließlich Belegnummer ab. Vorlagen für beide Textarten sind Teil des zentralen Settings-Datensatzes und keine zweite Fachstruktur. Details: [Behandlungsdokumentation](treatment-documentation.md).
...
MULTI-COMPANY-002 hebt 8→9 ohne neuen Store an. `companies[]` wird im Settings-Datensatz kanonisch; MULTI-COMPANY-004 verwaltet mehrere vollständig referenzierte Profile in der Produktoberfläche. `activeCompanyId` muss immer auf genau einen vorhandenen Eintrag zeigen und wird persistent als zentraler Arbeitskontext verwendet. Die stabile Profil-1-ID wird für den vorhandenen lokalen Namespace deterministisch als opake Referenz erzeugt. Unternehmensidentität, Anschrift/Kontakt, Brandingreferenz, `taxSettings`, `receiptSettings` samt Nummernstand, `paymentChoices`, `tseSettings`, portable `license`-Referenz und `setup` liegen ausschließlich im Profil. Benutzer, Logo-Asset-Register, Behandlungsvorlagen und Backup-Erinnerung bleiben auf Installationsebene; Geschäftsbereiche und Leistungsorte liegen ebenfalls im gemeinsamen Settings-Datensatz, tragen aber zwingend eine direkte `companyId` und werden nur im aktiven Profil bearbeitet.
...
- `companies` mit mindestens einem vollständig referenzierten Profil aus `id`, `formatVersion`, `company`, `taxSettings`, `receiptSettings`, `paymentChoices`, `tseSettings`, portabler `license`-Referenz und `setup` sowie eine passende `activeCompanyId`; die Produktoberfläche kann weitere Profile mit eigenem Kürzel anlegen und kontrolliert wechseln. Ausschließlich im Beta-Build darf ein Zusatzprofil zusätzlich den lokalen, nicht portablen Testzustand `betaProductiveTest` tragen;
...
SETTINGS-002 führt keinen neuen Store ein. Die Seite **Betrieb** bearbeitet die profilgebundenen `taxSettings`, `receiptSettings` und `paymentChoices` sowie den eindeutigen Eintrag `businessAreas[].isDefault`. EUR und Deutsch bleiben feste V1.0-Werte. `receiptSettings.yearPrefix` und `receiptSettings.nextNumber` werden nur als geschützte Ein-Profil-Projektion angezeigt; die seit MULTI-COMPANY-003 kanonischen Storno-/Gutschriftzähler sind dort ebenfalls nicht bearbeitbar. Auch ein erneut gestarteter Einrichtungsassistent darf keinen Nummernkontext zurücksetzen. Bei einem fehlgeschlagenen Speichervorgang werden die in der Betriebsseite versuchten Laufzeitänderungen auf den zuletzt bestätigten Stand zurückgesetzt.
...
Explizit konfigurierte Profile verwenden `FR-2030-000001`, `FR-ST-2030-000001` und `FR-GS-2030-000001`. Profilkürzel sind installationsweit eindeutig, großgeschrieben und vor produktivem Abschluss jedes weiteren Profils verpflichtend. Neue Profile starten standardmäßig je Typ und Jahr bei 1; ein expliziter anderer Startwert ist strukturell möglich. Profil 1 bleibt ohne geratenes Kürzel im Übergangsmodus und führt normale Nummern im bisherigen `JJJJ-NNNNNN`-Format fort. Historische `ST-/GS-`-Nummern bleiben unverändert; beim ersten Folgeabschluss wird ihr bisheriger Höchststand profilgebunden eingelesen. Das fachliche Jahr stammt vorrangig aus dem beim Abschluss gespeicherten lokalen Belegdatum und sonst aus `completedAt`, niemals aus Render- oder Exportzeit. So bleibt auch ein lokaler Jahreswechsel trotz UTC-Zeitstempel korrekt.

`commitReceipt` öffnet eine gemeinsame Readwrite-Transaktion über `settings` und `receipts`, löst Profil über `companyId`, Geschäftsbereich und Leistungsort auf, vergibt die nächste kollisionsfreie Nummer, schreibt den vollständigen Beleg und erhöht den betroffenen Profil-/Typ-/Jahreszähler. Storno und Gutschrift aktualisieren Settings, Ursprungsbeleg und Korrekturbeleg ebenfalls in einer gemeinsamen Transaktion. Erst `transaction.oncomplete` bestätigt den Vorgang. Bei Abbruch bleibt der Zähler unverändert; parallele Transaktionen werden durch IndexedDB plus vorhandene Schreibqueue serialisiert. Eine stabile Beleg-ID macht die Wiederholung desselben Abschlusses idempotent.

Neue Korrekturen speichern `originalReceiptId`/`correctionIds` als primäre interne Beziehungen und zusätzlich die unveränderten sichtbaren Nummern für Anzeige und Export. Der Legacy-Fallback über eine sichtbare Nummer ist auf dasselbe Profil begrenzt; eine mehrdeutige Referenz wird abgewiesen. Profil und alle historischen Snapshots einer Korrektur stammen ausschließlich vom Ursprungsbeleg, niemals vom aktiven UI-Kontext.

Storno- und Gutschriftsnummern verwenden ihren jeweils eigenen profil-/jahresbezogenen Zähler. Im Profilmodus lauten sie `<KÜRZEL>-ST-<Jahr>-<Sequenz>` und `<KÜRZEL>-GS-<Jahr>-<Sequenz>`; Profil 1 behält im Übergang das historische Format ohne Kürzel. Der vorhandene Receipt-Bestand ist zusätzlich eine Kollisionssicherung, aber keine konkurrierende Zählerquelle. Ursprungsbeleg, Folgedokument und betroffener Nummernkontext werden atomar aktualisiert. Die Dokument-Snapshots des Ursprungs bleiben unverändert; nur Lebenszyklusfelder wie Status, stabile Korrekturreferenzen, Aktivität und `updatedAt` werden ergänzt. Ein zweiter Stornoklick erzeugt kein Duplikat.
...
`commitVoucherSale` und `commitVoucherRedemption` verwenden dieselbe profilbezogene Normalbelegvergabe wie `commitReceipt`; Gutscheincodes bleiben davon unabhängig. Beide öffnen genau eine Readwrite-Transaktion über `settings`, `receipts` und `vouchers`. Ein Verkauf bestätigt deshalb Profilzähler, Verkaufsbeleg, Gutschein und ersten Historieneintrag gemeinsam. Eine Einlösung bestätigt Profilzähler, Einlösungsbeleg, Zahlungsstatus, neuen Restwert, Status, Einlösungsreferenz und Historieneintrag gemeinsam. Schlägt ein Teil fehl, wird die gesamte Transaktion abgebrochen; es gibt weder halbe Datensätze noch eine Nummernlücke.
...
Die kanonischen Receipt-Daten stammen ausschließlich aus einer getrennten Reparaturquelle mit exakt diesen vier Beleg-/Gutscheinpaaren. Sie gehören nicht zum aktiven Erststart-Seed. Vor jedem Schreibzugriff liest die Reparatur alle sieben Tenant-Stores innerhalb einer gemeinsamen Readwrite-Transaktion, sammelt sämtliche Gutschein-/Verkaufsbelegverletzungen und erzeugt zunächst einen vollständig validierten Kandidaten im Arbeitsspeicher. Geschrieben wird ausschließlich der Receipt-Store und nur dann, wenn nach dem Ergänzen aller fehlenden zulässigen Demo-Receipts der vollständige Tenant-Snapshot gültig ist. Einstellungen, profilbezogene Belegnummernfolgen, Katalog, Kunden, Gutscheine, vorhandene Receipts und neuere Gutscheinverkäufe werden nicht verändert.
...
- Historische Settings werden nur dann dauerhaft ergänzt, wenn ausschließlich bekannte additive Felder oder vollständig fehlende Vorgängermodelle betroffen sind und der fertige Datensatz anschließend ohne weiteren Reparaturcode vollständig validiert. Die atomare Settings-Transaktion verändert keinen anderen Store. Mehrdeutige Normalisierungen bleiben gesperrt.
...
Geprüft werden weiterhin alle Fälle aus PERSIST-001a bis PERSIST-004. PERSIST-005 ergänzt das Upgrade 4 → 5, Voucher-Roundtrip, Centwerte, QR- und Belegreferenzen, unveränderliche Snapshots und Historien, atomaren Verkauf, Teil- und Voll-Einlösung, idempotente Wiederholung, Duplikat- und Restwertschutz, Transaktionsfehler ohne halbe Datensätze oder Nummernverbrauch, erweiterte Kundensuche sowie den tenant- und storeisolierten Voucher-Reset. BACKUP-001 ergänzt Snapshot-, Verschlüsselungs-, Manipulations-, Export-, Restore- und Rollbackprüfungen einschließlich des Einspielens in einen leeren Mandanten. HARDEN-001 ergänzt den reversiblen Kundenstatus sowie Dateinamen- und Downloadtypprüfungen. EXPORT-001 ergänzt Zeitraum- und Geschäftsbereichsfilter, offene Zahlung, Storno, Gutschrift, Gutscheinverkaufsbeleg, Gutscheinhistorie, CSV-Regeln, Injection-Schutz, Steuerberater-ZIP, PDF-Einträge, Datenschutz und Snapshot-Unveränderlichkeit. EXPORT-003 ergänzt die positiven Demo- und End-to-End-Fälle sowie Negativfälle für fehlende Belege, falsche ID-/Nummernpaarung, falsche Belegart, falsche Gegenreferenz, verwaiste Verkaufsbelege, Restore und Export-Stopp. PERSISTENCE-007 ergänzt Schreibtests gegen einen unverändert inkonsistenten Altbestand, lokale Gegenreferenzprüfungen sowie den fortbestehenden Backup-, Export- und Restore-Stopp. PERSISTENCE-008 ergänzt die lokale Read-only-Diagnose. PERSISTENCE-010 ergänzt Vorprüfung aller vier bekannten Demo-Fälle, No-op, Teil- und Vollreparatur, ID-/Nummern-/Daten-/Referenz-/Mehrfachanspruch-Stopps, atomaren Rollback, Reload-Idempotenz, unveränderte Nummernfolge und Stores sowie erfolgreiche Snapshot-, Backup- und Steuerberaterexportpfade nach der Reparatur. SETTINGS-001 ergänzt Unternehmensfelder, Legacy-Anschrift, getrennten Änderungszeitpunkt, Logoformat und -Reload, Backup/Restore, Eigene-Daten-Export sowie die Public-QR-Datenschutzgrenze. SETTINGS-002 ergänzt die gebündelte Betriebsseite, den Nummernkreisschutz, die unveränderten Zahlungs-/Steuer-/Standardbereichsmodelle sowie die Abgrenzung des Eigene-Daten-Exports. TSE-002 ergänzt das deaktivierte Erlaubnislistenmodell, Legacy-Settings und -Backups, Restore, getrennte Exporttypen, Belegschutz und die rein lesende Einstellungsseite. BRANDING-002 ergänzt Registermigration, Deduplizierung, unveränderliche historische Versionen, zentrale Auflösung, PNG/JPEG-PDFs, Fallback, Backup/Restore und Exportdatenschutz. BETA-HANDOFF-001 ergänzt frische Erststartprojektionen, getrennte Reparaturquellen, erste Nummern und den Vorrang vorhandener Mandantendaten. BACKUP-006 ergänzt acht historische Settingsprofile, Kombination mit PERSISTENCE-010, Reihenfolgeregression, atomare Startpersistenz, AES-GCM-/Restore-Roundtrip, Idempotenz, `fail closed` und die Diagnose-Datenschutzgrenze. LICENSE-005 ergänzt V1→V2-Migration, Schema 5→6, Runtime-Isolation, P-256-Schlüsselreload, strikte JWS-Grenze sowie Backup-/Restore-/Exportausschlüsse. MULTI-COMPANY-003 ergänzt getrennte Profil-/Typ-/Jahreszähler, Übergangsmodus, Parallelität, Jahreswechsel, stabile Korrektur-IDs und Restore-Fortsetzung. QR-001 ergänzt zentrale Service-, App-Link-, Matrix-, SVG-, Suchmuster- und Fehlerfallprüfungen. Der aktuelle native Browserlauf umfasst 269 Fälle.

Die unmittelbar vorausgehende Zählung von 269 Fällen bezeichnet den Stand vor MULTI-COMPANY-004. Der aktuelle native Browserlauf umfasst 277 Fälle. MULTI-COMPANY-004 ergänzt Profilanlage und -wechsel, Kürzelvalidierung und Profil-1-Fortsetzung, lokale Produktsperre, aktive Exportbegrenzung, Mehrprofil-Backup/Restore, globale Kunden sowie den dauerhaft deaktivierten Unternehmensanschrift-Leistungsort-Schalter.

BACKUP-005 ergänzt die zuvor fehlende dauerhafte BRANDING-002-Startmigration: Ein vorhandener BRANDING-001-Settingssatz ohne `logoAssets` beziehungsweise mit Inline-Logo wird in das bestehende Asset-Register normalisiert und anschließend im selben Settings-Store gespeichert. Die Snapshotkompatibilität führt dieselbe verlustfreie Projektion vor Backup/Restore aus, sodass ein noch nicht migrierter Altsatz nicht mehr allein wegen der vorgesehenen Logo-Normalisierung an `BACKUP_VALIDATION_FAILED` scheitert. Ungültige, mehrdeutige oder zu große Logos werden dadurch nicht akzeptiert; die bestehenden Validierungsgrenzen bleiben bestehen. Der BACKUP-005-Browserlauf umfasst 195 Fälle.
...
„Gespeicherten Katalog zurücksetzen“ löscht ausschließlich den Katalogdatensatz der aktuellen `tenantId`. Settings, Kunden, Belege und Gutscheine bleiben unverändert. Anschließend ist die zentrale Katalogquelle leer; Vorlagen werden nicht automatisch importiert. Alle Resetaktionen sind getrennt bestätigt und führen keine versteckte Löschung aus.

„Gespeicherte Kunden zurücksetzen“ löscht ausschließlich den Kundendatensatz der aktuellen `tenantId`. Settings, Katalog, Belege und Gutscheine bleiben unverändert. Anschließend ist die zentrale Kundenquelle leer. Die Aktion besitzt eine eigene Bestätigung und löscht keine Datensätze anderer Mandanten.

„Gespeicherte Belege zurücksetzen“ löscht ausschließlich den Receipt-Datensatz der aktuellen `tenantId`. Settings, Katalog, Kunden und Gutscheine bleiben unverändert. Der Settings-Nummernstand wird absichtlich nicht abgesenkt, damit kein zuvor verwendeter Nummernstand erneut vergeben wird. Anschließend ist die zentrale Belegquelle leer.

„Gespeicherte Gutscheine zurücksetzen“ löscht ausschließlich den Voucher-Datensatz der aktuellen `tenantId`. Einstellungen, Nummernstand, Katalog, Kunden und Belege bleiben unverändert. Anschließend ist die zentrale Gutscheinquelle leer.
