...
Das real abgenommene Artefakt `0.11.1-3a4ff57` / `BETA-HANDOFF-001` bleibt die unmittelbare freigegebene Beta-Basis. Die bereitgestellten Releases `0.11.2-a959ec6` / `ONBOARDING-001` und `0.11.3-a8380a0` / `BACKUP-005` sind nach real fehlgeschlagenen iPhone-Backuptests gesperrt. Der vorbereitete Patch-Kandidat `0.11.4` / `BACKUP-006` ergänzt gegenüber 0.11.3 ausschließlich den semantischen Remindervergleich, die streng nachvalidierte historische Settingsnormalisierung und die datenschutzgehärtete lokale Diagnose sowie notwendige Versions-, Cache-, Test- und Releaseanpassungen; seine Freigabenotiz steht unter `docs/releases/0.11.4.md`. Backupformat, Datenbankschema, AES-GCM/PBKDF2, Geschäftsdaten sowie der SERVICEWORKER-003-Regressionsschutz bleiben unverändert.
