...
Der annotierte Tag `v0.10.8` zeigt auf den vollständigen Versionsstand `6056e64`; sein unveränderliches Artefakt bleibt wegen des abgelehnten realen Updateabschlusses unverändert. Der korrigierte Tag `v0.10.9` zeigt auf `5b180b64ec75ab6f6c2ef53842ead45c6cc32b4a` und bestand den realen Update- und Offlinepfad. Der annotierte Tag `v0.10.10` zeigt auf `b3456bf1b5f4e6fff074e97acbaea994d064daac`; das unveränderliche Artefakt `0.10.10-b3456bf` validierte anschließend SERVICEWORKER-003 und den Ein-Befehl-Releaseprozess real. Der annotierte Tag `v0.11.0` zeigt auf `99a15113cfd589258928cad7b824f857c2be24d9`; das real abgenommene Artefakt `0.11.0-99a1511` bleibt unverändert erhalten. Der annotierte Tag `v0.11.1` zeigt auf `3a4ff5768afc1aebfbc75ca700c1854d3476ccb6`; das real abgenommene Artefakt `0.11.1-3a4ff57` bleibt unverändert erhalten. Der annotierte Tag `v0.11.4` zeigt auf `e628c116682cc76b33fb6273191eef4c82875946`; das real abgenommene Artefakt `0.11.4-e628c11` bleibt unverändert erhalten. `v0.11.5` zeigt auf `c80edc7d63042c9f8762a751d376d2d4dacb884d`, `v0.11.6` auf `ee13b12ed7cd4763d23d6f4979b83cd5a75ebbef` und `v0.11.7` auf `608257fc99b7f1fd93d893cc6104ce55e6e40f44`; die Artefakte `0.11.5-c80edc7`, `0.11.6-ee13b12` und `0.11.7-608257f` bleiben ebenfalls unverändert. Historische Tags und Artefakte werden durch RELEASE-AUTOMATION-001 weder umbenannt noch neu erzeugt.

Das unveränderliche Artefakt `0.11.6-ee13b12` / `ANDROID-002` ist die bestätigte Beta-Basis vor den Podologie-Blöcken. Die bereitgestellten Releases `0.11.2-a959ec6` / `ONBOARDING-001` und `0.11.3-a8380a0` / `BACKUP-005` bleiben nach real fehlgeschlagenen iPhone-Backuptests gesperrt. `0.11.7-608257f` / `PODOLOGY-004` ergänzt gegenüber 0.11.6 PODOLOGY-001 bis PODOLOGY-004; das Datenbankschema stieg dabei additiv von 6 auf 8. Die veröffentlichten Releases `0.11.8-d9093e7` / `PODOLOGY-005`, `0.11.9-2d6b2d4` / `PODOLOGY-006` und `0.11.10-b6512dd` / `BETA-PREVIEW-001` bleiben unverändert. `0.11.11` / `BETA-PREVIEW-002` bündelt MULTI-COMPANY-002/003/004; seine strukturierte Freigabenotiz steht unter `docs/releases/0.11.11.md`. Der vorbereitete Folgestand `0.11.12` / `BETA-PREVIEW-003` ergänzt ausschließlich MULTI-COMPANY-004B und nimmt die neue Buildkennung explizit in dieselbe eng begrenzte Beta-Test-Allowlist auf. Seine strukturierte Freigabenotiz steht unter `docs/releases/0.11.12.md`. Sie erlaubt nach Commit und Push den automatisierten Beta-Release, ersetzt aber weder Pflichtsicherung, reale Geräteabnahme noch eine Produktivfreigabe. Schema 9, Backupformat, AES-GCM/PBKDF2, `licenseRuntime`, Service-Worker-Architektur und SERVICEWORKER-003-Regressionsschutz bleiben unverändert.
