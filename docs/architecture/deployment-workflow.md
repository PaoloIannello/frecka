...
Das unveränderliche Artefakt `0.11.6-ee13b12` / `ANDROID-002` ist die bestätigte Beta-Basis vor den Podologie-Blöcken. Die bereitgestellten Releases `0.11.2-a959ec6` / `ONBOARDING-001` und `0.11.3-a8380a0` / `BACKUP-005` bleiben nach real fehlgeschlagenen iPhone-Backuptests gesperrt. `0.11.7-608257f` / `PODOLOGY-004` ergänzt gegenüber 0.11.6 PODOLOGY-001 bis PODOLOGY-004; das Datenbankschema stieg dabei additiv von 6 auf 8. Die veröffentlichten Releases `0.11.8-d9093e7` / `PODOLOGY-005`, `0.11.9-2d6b2d4` / `PODOLOGY-006` und `0.11.10-b6512dd` / `BETA-PREVIEW-001` bleiben unverändert. Der vorbereitete Multi-Company-Zwischen-Beta-Stand `0.11.11` / `BETA-PREVIEW-002` bündelt MULTI-COMPANY-002/003/004 sowie notwendige Versions-, Cache-, Test- und Releaseanpassungen; seine strukturierte Freigabenotiz steht unter `docs/releases/0.11.11.md`. Sie erlaubt nach Commit und Push den automatisierten Beta-Release, ersetzt aber weder die Pflichtsicherung unter 0.11.10, die reale Schema-8→9-Migration und Geräteabnahme noch eine Produktivfreigabe. Schema 9 kommt ohne neuen Store; Backupformat, AES-GCM/PBKDF2, `licenseRuntime`, Service-Worker-Architektur und SERVICEWORKER-003-Regressionsschutz bleiben unverändert.
