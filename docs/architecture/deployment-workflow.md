...
Das real abgenommene Artefakt `0.11.1-3a4ff57` / `BETA-HANDOFF-001` bleibt die unmittelbare freigegebene Beta-Basis. Das bereitgestellte Release `0.11.2-a959ec6` / `ONBOARDING-001` ist wegen des real fehlgeschlagenen iPhone-Backups gesperrt. Der vorbereitete Patch-Kandidat `0.11.3` / `BACKUP-005` ergänzt gegenüber 0.11.2 ausschließlich die dauerhafte und snapshotkompatible Übernahme historischer BRANDING-001-Inline-Logos sowie notwendige Versions-, Cache-, Test- und Releaseanpassungen; seine Freigabenotiz steht unter `docs/releases/0.11.3.md`. Backupformat, Datenbankschema, Verschlüsselung, BACKUP-004-Reminder sowie vorhandene Mandanten- und Beta-Daten bleiben unverändert. Der SERVICEWORKER-003-Regressionsschutz gegen die entfernte Legacy-Brücke bleibt unverändert aktiv.
