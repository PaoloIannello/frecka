...
MULTI-COMPANY-004B behält Schema 9, Lizenzreferenzen und `licenseRuntime` unverändert. Ein Zusatzprofil kann nur im ausdrücklich erlaubten Build `BETA-PREVIEW-002` den lokalen Testzustand `betaProductiveTest` erhalten. Der echte Status bleibt `activation_required`; die zentrale Produktivprüfung akzeptiert ausschließlich `regularProductive` oder die Kombination aus gespeichertem Testzustand und erlaubter Buildkennung. Ein Nicht-Beta-Build ignoriert das Flag. Tenant-Snapshot, Backup, Restore, Diagnose, Export, PDF und Public-QR erhalten es nicht. Deaktivierung sperrt nur künftige Mutationen und verändert weder Historie noch Nummernkreise. Die temporäre Entscheidung steht in [ADR-0009](docs/adr/ADR-0009-temporaere-beta-testfreigabe-fuer-zusatzprofile.md).
