...
Die veröffentlichte Version `0.11.1` mit Build `BETA-HANDOFF-001` ergänzt gegenüber `0.11.0-99a1511` ausschließlich BRANDING-001, BRANDING-002 und BETA-HANDOFF-001. Ein neuer Mandant startet ohne Demo-Geschäftsdaten und ohne Logoassets; vorhandene Mandanten- und Beta-Daten behalten beim Laden Vorrang und werden weder automatisch gelöscht noch zurückgesetzt. Unternehmen und Geschäftsbereiche können lokale PNG-/JPEG-Logos verwenden; ein unveränderliches Asset-Register erhält die historisch referenzierte Bildversion für interne Ansicht und PDF. Public-QR bleibt frei von Bildrohdaten. TSE bleibt optional vorbereitet und ist weder aktiviert noch verbunden oder fiskalisiert. Release `0.11.2` mit Build `ONBOARDING-001` ergänzt BACKUP-004 und ONBOARDING-001, bleibt nach dem realen iPhone-Backupfehler jedoch NO-GO. Auch `0.11.3` mit Build `BACKUP-005` bleibt nach dem realen In-place-Test NO-GO: Das historische Reminderfeld wurde inhaltlich korrekt ergänzt, aber eine reine Eigenschaftsreihenfolge wurde fälschlich als Inkonsistenz gewertet. `0.11.4-e628c11` mit Build `BACKUP-006` korrigierte diesen Vergleich und bestand auf dem realen iPhone mit historischem Bestand Backup, Restore und Offline-Kaltstart; es ist die aktuelle stabile Beta-Basis. Der vorbereitete Geräteprüfungsstand `0.11.5` mit Build `LICENSE-005` ergänzt gegenüber 0.11.4 ausschließlich DOCUMENT-001 und die getrennte lokale Lizenzruntime samt notwendigen Versions-, Cache-, Diagnose-, Test- und Releaseanpassungen. Eine erfolgreiche iPhone- oder Android-CryptoKey-Abnahme wird nicht vorweggenommen.

## 1. Projektvision

Aktueller Entwicklungszusatz PODOLOGY-001/002/003/004 auf Basis von v0.11.6: `features.prescriptionDocumentation` ist pro Geschäftsbereich optional und standardmäßig aus. Rezepte sind ein eigener mandantenbezogener Bestand (`prescriptions`, Schema 7), nicht Teil von Kundenobjekten. Ein normaler Beleg kann beim Abschluss optional einen unveränderlichen Rezept-Snapshot aufnehmen. Der Verbrauch wird ausschließlich aus diesen abgeschlossenen Belegen abgeleitet; es gibt weder gespeicherte Restwerte noch eine zweite Nutzungsliste. PODOLOGY-003 ergänzt in Schema 8 genau einen getrennten mandantenbezogenen Bestand `treatmentRecords`. Ein Behandlungsdatensatz entsteht ausschließlich zusammen mit einem erfolgreich abgeschlossenen normalen Ursprungsbeleg und wird in derselben IndexedDB-Transaktion geschrieben; ohne Inhalt entsteht kein Datensatz. PODOLOGY-004 projiziert ausschließlich in das lokale Kundendokument eines normalen Belegs das historische Rezeptdatum und den exakt beleggebundenen Kundenpflegehinweis. Interne Behandlungsdokumentation bleibt immer intern; Public Viewer, QR-Payload, Steuerberaterausgabe, reguläre Exporte, Diagnose und Logs bleiben frei von diesen Angaben. Vorlagen liegen ausschließlich in den zentralen Einstellungen. Snapshot, verschlüsseltes Backup, Restore und Integritätsprüfung umfassen sieben Fachstores; `licenseRuntime` bleibt getrennt. Verbindliche Details: [Rezeptverwaltung](docs/prescriptions.md) und [Behandlungsdokumentation](docs/treatment-documentation.md).
