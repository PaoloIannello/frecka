...
Dieses Dokument beschreibt die verbindliche Zielrichtung von FRECKA. Der aktuelle Stand ist ein browserbasierter Prototyp mit lokaler IndexedDB-Persistenz für Einstellungen, Katalog, Kunden, Belege und Gutscheine sowie manueller verschlüsselter Gesamtsicherung und atomarer Wiederherstellung. COMM-001 / QR-002 ergänzt einen zentralen, zustandslosen Share-Service und geräteübergreifende Kundenlinks mit einer datensparsamen Dokumentprojektion im URL-Fragment; eine zentrale Belegablage entsteht dadurch nicht. OFFLINE-001 ergänzt eine versionierte statische App-Shell mit Service Worker und Navigation-Fallback für den Offline-Kaltstart; Geschäftsdaten bleiben davon getrennt in IndexedDB. Der reale iPhone-Beta-Smoke-Test des Releases `0.9.1-26dc63f` hat Online-Start, Offline-Kaltstart, lokale Datennutzung und einen vollständig offline erzeugten, nach Rückkehr ins Netz weiterhin vorhandenen Beleg bestätigt. Das getaggte und auf Beta bereitgestellte Release `0.10.4-2d5d3c4` mit Build `BACKUP-001` machte den gesperrten Backup-Fehlerpfad verständlich, zeigte im realen iPhone-Test jedoch einen verzögerten Systemdialog nach einer abgelehnten Sicherung. Version `0.10.5` mit Build `BACKUP-002` trennt Backup-Vorbereitung und ausdrückliche Ausgabe lifecycle-sicher. Version `0.10.6` mit Build `PERSISTENCE-008` ergänzt eine ausschließlich lokale, begrenzte Read-only-Diagnose für Snapshot-Inkonsistenzen. Version `0.10.7` mit Build `PERSISTENCE-010` ergänzt darauf ausschließlich eine bewusst ausgelöste, feste Vierer-Allowlist-Reparatur für eindeutig bekannte historische Demo-Gutscheinverkaufsbelege; sie ist keine allgemeine Migration und rekonstruiert keine realen Geschäftsdaten. Release `0.10.8-6056e64` mit Build `UPDATE-001` wurde am 11. August 2026 real auf einem iPhone als Home-Screen-PWA getestet: Updateerkennung und Offline-Kaltstart bestanden, der bewusste Updateabschluss blieb jedoch ohne sichtbaren Reload hängen. 0.10.8 erhielt deshalb kein Beta-GO. Das Folge-Release `0.10.9-5b180b6` mit Build `UPDATE-001b` hat den realen iPhone-Updatepfad, Offline-Kaltstart, Offline-Belegerstellung und den fortbestehenden lokalen Datenbestand nach Rückkehr ins Netz bestanden. Release `0.10.10-b3456bf` mit Build `SERVICEWORKER-003` entfernte danach die bestätigte einmalige Legacy-Brücke und validierte den lokalen Ein-Befehl-Releaseprozess real. Das real abgenommene Release `0.11.0-99a1511` mit Build `TSE-002` war die unmittelbare Basis für das nachfolgende, veröffentlichte und real abgenommene Beta-Release `0.11.1-3a4ff57` mit Build `BETA-HANDOFF-001`; eine Produktivfreigabe für `app.frecka.app` folgt daraus nicht. Seit SERVICEWORKER-003 gilt dauerhaft Hinweis, Nutzeraktion, `SKIP_WAITING` und genau ein Reload. RELEASE-AUTOMATION-001 führt für künftige bereits geprüfte und gepushte Release-Commits einen lokalen Ein-Befehl-Prozess für Preflight, Tests, annotierten Tag, reproduzierbares Artefakt und Beta-Upload ein; Web Station und reale Geräteabnahme bleiben manuelle Gates. Aussagen zu noch offenen Teilen der Zielarchitektur kennzeichnen nicht automatisch bereits implementierte Funktionen. Abweichungen von diesen Leitlinien benötigen eine dokumentierte Architekturentscheidung (ADR) mit Begründung, Folgen und Migrationsweg.

Die veröffentlichte Version `0.11.1` mit Build `BETA-HANDOFF-001` ergänzt gegenüber `0.11.0-99a1511` ausschließlich BRANDING-001, BRANDING-002 und BETA-HANDOFF-001. Ein neuer Mandant startet ohne Demo-Geschäftsdaten und ohne Logoassets; vorhandene Mandanten- und Beta-Daten behalten beim Laden Vorrang und werden weder automatisch gelöscht noch zurückgesetzt. Release `0.11.2` mit Build `ONBOARDING-001` und `0.11.3` mit Build `BACKUP-005` bleiben nach real fehlgeschlagenen iPhone-Backuptests NO-GO. `0.11.4-e628c11` mit Build `BACKUP-006` korrigierte den historischen Settingsvergleich und bestand Backup, Restore und Offline-Kaltstart. Das unveränderliche Release `0.11.5-c80edc7` ergänzte DOCUMENT-001 und LICENSE-005; `0.11.6-ee13b12` mit ANDROID-001, IOS-NAV-001 und ANDROID-002 ist die bestätigte Beta-Basis vor den Podologie-Blöcken. `0.11.7-608257f / PODOLOGY-004` bündelt PODOLOGY-001 bis PODOLOGY-004. Das veröffentlichte Release `0.11.8-d9093e7 / PODOLOGY-005` verbessert ausschließlich Vorlagenauswahl, Hilfetexte und Vorlagenkarten; `0.11.9-2d6b2d4 / PODOLOGY-006` ergänzt den kompakten internen Behandlungsverlauf. `0.11.10-b6512dd / BETA-PREVIEW-001` bündelt darauf ANDROID-004, COMPLIANCE-002 und DOCUMENT-002. `0.11.11 / BETA-PREVIEW-002` nimmt zusätzlich MULTI-COMPANY-002/003/004 für die reale Schema-8→9-Migration und Profilprüfung auf. Der vorbereitete Stand `0.11.12 / BETA-PREVIEW-003` ergänzt darauf ausschließlich MULTI-COMPANY-004B und die ausdrückliche Buildfreigabe für den kontrollierten Test produktiver Fachpfade eines Zusatzprofils. Technische Release-Freigabe, reale Geräteabnahme und Produktivfreigabe bleiben getrennte Gates.

MULTI-COMPANY-002/003/004 bilden den Fachstand der 0.11.11-Zwischen-Beta. Schema 9 macht `companies[]` zur kanonischen Unternehmensstruktur; MULTI-COMPANY-003 ergänzt darin versionierte, profil- und jahresbezogene Nummernkontexte für Normalbeleg, Storno und Gutschrift. MULTI-COMPANY-004 macht die Profile in den Einstellungen verwaltbar, verwendet `activeCompanyId` als zentralen Arbeitskontext und verlangt für jedes neue Profil ein explizites, installationsweit eindeutiges Belegkürzel. Geschäftsbereiche, Leistungsorte, Belege, Gutscheine, Rezepte und Behandlungsdatensätze tragen eine direkte, hart validierte `companyId`; Kunden, V1-Benutzer, Logo-Asset-Register und Geräteruntime bleiben installationsweit gemeinsam. Ein offener Entwurf blockiert den Profilwechsel. Historische Snapshots, Nummern, Zeitpunkte und Steuerwerte werden nicht neu erzeugt oder verändert. Bis zu einem eigenen Lizenz-Multi-Binding-Block ist regulär nur Profil 1 produktiv; weitere Profile bleiben `activation_required`. MULTI-COMPANY-004B erlaubt ausschließlich in den explizit freigegebenen kanonischen Builds `BETA-PREVIEW-002` und `BETA-PREVIEW-003` eine getrennte, profilbezogene und installationslokale Testfreigabe am zentralen Write-Guard. Sie ist keine Lizenz, wird nicht portabel gesichert und ist in allen anderen Builds wirkungslos. Profilübergreifende Beleglisten und Exporte folgen erst in MULTI-COMPANY-005.

## 1. Projektvision

Version 0.11.12 führt die in 0.11.11 enthaltene Architektur fort und ergänzt MULTI-COMPANY-004B für den kontrollierten Multi-Company-Betatest; die zugrunde liegenden Fachstände bleiben MULTI-COMPANY-002/003/004: `features.prescriptionDocumentation` ist pro Geschäftsbereich optional und standardmäßig aus. Rezepte sind ein eigener mandantenbezogener Bestand (`prescriptions`, Schema 7), nicht Teil von Kundenobjekten. Ein normaler Beleg kann beim Abschluss optional einen unveränderlichen Rezept-Snapshot aufnehmen. Der Verbrauch wird ausschließlich aus diesen abgeschlossenen Belegen abgeleitet; es gibt weder gespeicherte Restwerte noch eine zweite Nutzungsliste. PODOLOGY-003 ergänzt in Schema 8 genau einen getrennten mandantenbezogenen Bestand `treatmentRecords`. Ein Behandlungsdatensatz entsteht ausschließlich zusammen mit einem erfolgreich abgeschlossenen normalen Ursprungsbeleg und wird in derselben IndexedDB-Transaktion geschrieben; ohne Inhalt entsteht kein Datensatz. PODOLOGY-004 projiziert ausschließlich in das lokale Kundendokument eines normalen Belegs das historische Rezeptdatum und den exakt beleggebundenen Kundenpflegehinweis. PODOLOGY-005 verbessert ausschließlich die UX der Vorlagenauswahl, kontextbezogene Hilfetexte und die Kompaktheit der Vorlagenkarten. PODOLOGY-006 zeigt Behandlungen im Kundenprofil initial geschlossen; mehrere Einträge sind unabhängig aufklappbar, ohne persistierten UI-Zustand. Das verbessert die visuelle Diskretion, ist aber keine Zugriffskontrolle. ANDROID-004 härtet ausschließlich die bewusste lokale Backup-Ausgabe; COMPLIANCE-002 dokumentiert Betriebsmodell und TSE-Abgrenzung ohne pauschale Befreiungsbehauptung; DOCUMENT-002 ergänzt den history-sicheren Leistungsort und die effektive Steuernummer auf neuen Belegen. Interne Behandlungsdokumentation bleibt immer intern; Public Viewer, QR-Payload, Steuerberaterausgabe, reguläre Exporte, Diagnose und Logs bleiben frei von diesen Angaben. Vorlagen liegen ausschließlich in den zentralen Einstellungen. Snapshot, verschlüsselte Sicherung, Wiederherstellung und Integritätsprüfung umfassen sieben Fachstores; `licenseRuntime` bleibt getrennt. Verbindliche Details: [Rezeptverwaltung](docs/prescriptions.md), [Behandlungsdokumentation](docs/treatment-documentation.md), [Compliance-Abgrenzung](docs/compliance-v1.md) und [Release 0.11.12](docs/releases/0.11.12.md).
...
Für V1.0 gilt das in [COMPLIANCE-002](docs/compliance-v1.md) definierte Betriebsmodell: FRECKA dokumentiert lokal Geschäftsvorfälle, Einnahmen und Zahlungsarten, ersetzt aber weder Kassenbuch, vollständige Kassenführung, Kassenbericht, Kassensturz noch Buchhaltung. Die TSE-Einordnung beruht auf der individuellen steuerberaterlichen Bewertung dieses konkreten Einsatzes und ist keine allgemeine TSE-Befreiung für FRECKA oder seine Anwender.
...
- Unternehmensbezogene Daten werden ab Schema 9 ausschließlich über zentrale Profilresolver aufgelöst. Fehlende oder profilfremde `companyId`-Referenzen sind Integritätsfehler; ein Profil-1-Fallback ist nur innerhalb der kontrollierten Schema-8→9-Migration zulässig.
...
Seit PERSIST-001a speichert der Prototyp die vollständigen Einstellungen über eine zentrale, versionierte IndexedDB-Schicht. PERSIST-002 ergänzt den Katalog, PERSIST-003 die Kundenstammdaten, PERSIST-004 normale Belege, offene Zahlungen, Stornos sowie Gutschriften und PERSIST-005 Gutscheine samt Historie. Verkauf und Einlösung bestätigen Nummernstand, Beleg, Gutschein und Historie atomar. BACKUP-001 ergänzt einen vollständigen Tenant-Snapshot, eine lokal verschlüsselte `.frecka-backup`-Datei und einen atomaren Restore aller fünf Stores. BACKUP-002 trennt die validierte und verschlüsselte Vorbereitung von der ausschließlich ausdrücklich ausgelösten File-/Share-/Download-Ausgabe und verwirft veraltete Ausgabezustände bei Fehlern oder Navigation. PERSISTENCE-008 verwendet dieselbe zentrale Snapshot- und Gutschein-/Beleg-Validierung für einen read-only gelesenen, datensparsamen Diagnosebericht. PERSISTENCE-010 verwendet denselben Tenant-Snapshot für eine explizite, idempotente und atomare Reparatur ausschließlich der vier freigegebenen historischen Demo-Receipt-Identitäten; jede Kollision, Mehrdeutigkeit oder weitere Verletzung stoppt vor dem Schreiben. HARDEN-001 macht Kunden deaktivierbar statt löschbar und härtet Sicherungskennwort, Dateiname und iOS-Dateiauswahl UX-seitig. EXPORT-001 verwendet denselben geprüften Tenant-Snapshot für eine zentrale, formatneutrale Exportprojektion und sichere CSV-Dateien; der Steuerberaterweg ergänzt daraus lokal `Übersicht.csv`, Beleg-PDFs und ein einziges ZIP-Gesamtpaket, ohne neue Stores oder eigene Sammelroutinen. QR-001 ergänzt einen einzigen laufzeitbasierten QR-Service für stabile Beleg- und Gutschein-App-Links. QR-Matrizen und SVGs werden nicht gespeichert; Beleg, Gutschein und spätere Ausgabewege verwenden dieselbe API. DOCUMENT-001 ergänzt daraus reine Beleg- und Gutscheindokumentmodelle sowie echte lokale PDFs mit durchsuchbarem Text und vektoriellen QR-Codes. Bildschirmvorschau und PDF verwenden dieselbe Projektion; PDFs werden nicht persistiert. COMM-001 / QR-002 ergänzt Web-Share-Feature-Detection mit PDF-, Link- und Download-Fallbacks sowie das versionierte Public-Format `FPD/v1`. Beleg- und Gutscheindaten werden dafür ausschließlich als validierte Whitelist im Fragment eines statischen Viewer-Links transportiert; Share-Service und Viewer besitzen weder eine eigene Fachdatenstruktur noch Persistenz. UPDATE-001 ergänzt den bestehenden PWA-Updatecontroller um eine gemeinsame Einmal-Reload-Sperre für `activated` und `controllerchange`, begrenzte Fehlerzustände und eine rein sitzungsbezogene Erinnerung. UPDATE-001b ergänzt innerhalb eines ausdrücklich gestarteten Updateversuchs eine begrenzte Verifikation von Worker-, Registration- und Controllerstand, damit ein bereits vergangenes oder ausgebliebenes Lifecycle-Ereignis den Abschluss nicht blockiert. Beide Blöcke ändern weder Service-Worker-Fachdatenzugriffe noch IndexedDB. Belegentwürfe bleiben bis zu ihrem Persistenzblock ausschließlich im Arbeitsspeicher. Dieser Zwischenstand darf nicht mit der vollständigen Zielarchitektur verwechselt werden.

LICENSE-005 ergänzt die getrennte lokale Lizenzruntime: Die portable, nicht autoritative Lizenzreferenz Version 2 liegt ab MULTI-COMPANY-002 unter `settings.companies[].license`, während Geräte-ID, nicht exportierbarer P-256-Privatkey, exportierbarer Public Key, Thumbprint und optionale Token-/Zeitmetadaten ausschließlich im IndexedDB-Store `licenseRuntime` liegen. LICENSE-005 hob das Schema von 5 auf 6 an; MULTI-COMPANY-002 verschiebt nur die portable Referenz in Profil 1 und lässt Runtime, Schlüssel und Autoritätslogik unverändert. Der strikte ES256/JWS-Prüfpfad enthält keinen eingebauten Serverprüfschlüssel und erzeugt daher weder Trial-/Active-Status noch Produktsperre. LICENSE-006 bleibt bis zur realen CryptoKey-Persistenzprüfung auf iPhone/Home-Screen-PWA und Android gesperrt.

SETTINGS-001 definiert das Unternehmensfeldmodell, das seit MULTI-COMPANY-004 je Profil im selben Settings-Datensatz gilt. Optionale Geschäftsbezeichnung, verpflichtende rechtliche Person und optionaler Ansprechpartner bleiben getrennt. Historische kombinierte Straßenangaben werden nicht automatisch zerlegt; neu bearbeitete Anschriften können Straße und Hausnummer getrennt führen. BRANDING-001 macht validierte PNG-/JPEG-Logos bis 1 MB für Unternehmen und Geschäftsbereiche praktisch nutzbar. BRANDING-002 führt dafür innerhalb desselben Settings-Datensatzes ein versioniertes, unveränderliches Logo-Asset-Register ein. Aktive Zuordnungen und neue Geschäftsvorgänge speichern nur die stabile Asset-ID samt Metadaten; das Bild liegt je Version genau einmal im Register. Der zentrale Resolver liefert interner Ansicht, Dokumentvorschau, Beleg-/Gutschein-PDF und Steuerberater-PDF exakt die historische Version. Entfernen oder Ersetzen einer aktiven Zuordnung löscht keine historischen Assets. Verschlüsseltes Backup und Restore enthalten das Register vollständig; Public-QR und Exporte bleiben frei von Bildrohdaten.

SETTINGS-002 bündelt die bereits vorhandenen betrieblichen Vorgaben ohne neues Fachmodell unter **Einstellungen → Betrieb**. Zahlungsarten behalten IDs und Reihenfolge, `taxSettings.defaultRate` bleibt ausschließlich die Vorauswahl für neue Katalogeinträge, `businessAreas[].isDefault` bleibt die einzige Standardbereichsquelle und EUR die feste V1.0-Währung. Belegtexte werden nur in neue Dokument-Snapshots übernommen. Produktiv verwendete Belegnummern und die seit MULTI-COMPANY-003 getrennten Storno-/Gutschriftzähler sind in der normalen Oberfläche und im erneut gestarteten Assistenten nicht bearbeitbar.

UX-011 / UPDATE-002 bildet Benutzer und Update als reale Einstellungsbereiche ab. TSE-002 ergänzt eine reale, rein lesende TSE-Vorbereitungsseite und streng begrenzte lokale Konfigurationsmetadaten im bestehenden Settings-Datensatz. TSE ist im definierten V1.0-Betriebsmodell nicht aktiv; `fiskaly SIGN DE` bleibt der vorgesehene Anbieter einer eigenständigen späteren Erweiterung, aber weder Aktivierung noch Verbindung, Zugangsdaten, Transaktionen oder Fiskalisierung sind Bestandteil dieses Stands. Die sichtbare Vorbereitung ist weder eine allgemeine TSE-Pflicht noch eine allgemeine TSE-Befreiung. Betriebsmodell und individuelle Einordnung stehen in `docs/compliance-v1.md`, der technische Vertrag und die Abgrenzung zu Folgeblöcken in `docs/tse.md`. Die manuelle Updateprüfung verwendet dieselbe Service-Worker-Registration und denselben kontrollierten UPDATE-001b-Aktivierungspfad wie die automatische Updatekarte. BACKUP-003 ergänzt im vorhandenen Settings-Datensatz ausschließlich gerätelokale Erinnerungsmetadaten: sieben Tage ab lokaler Initialisierung beziehungsweise letzter bestätigter Sicherung und 24 Stunden nach „Später erinnern“. Erst eine tatsächlich an Share oder Download übergebene Sicherungsdatei setzt die Frist zurück. Restore ersetzt diesen lokalen Erinnerungsstatus nicht durch Metadaten einer älteren Sicherungsdatei.

BACKUP-004 erweitert die Erinnerung ohne neuen Store um genau eine lokale Intervallwahl: alle 48 Stunden, alle 5 Tage oder wöchentlich. Wöchentlich bleibt der Standard für neue und historische Daten. Intervallwechsel verändern weder letzte erfolgreiche Sicherung noch Snooze; ausschließlich eine bestätigte Share-/Download-Ausgabe startet die Frist neu. Die Speicherhilfe bleibt reine Bedieninformation ohne Cloudzugriff oder Integration.

ONBOARDING-001 nutzt ausschließlich die vorhandene Seite **Einstellungen → Hilfe & Lernen** für eine nicht blockierende, lokal verfügbare Installationshilfe. Eine datensparsame Geräteerkennung priorisiert iPhone/iPad oder Android und erkennt den Standalone-Modus; sie wird weder gespeichert noch übertragen. Beide Anleitungen bleiben manuell auswählbar. Nummerierte Schritte und lokale CSS-Piktogramme erklären Safari/Teilen/Home-Bildschirm beziehungsweise Chrome/Menü/Installation auch ohne Farbe. Die Anleitung ist unter `docs/installation.md` zusammengefasst und benötigt weder neue Laufzeitdateien noch eine Änderung der Service-Worker-Architektur.
...
MULTI-COMPANY-002 definiert Schema 9 ohne neuen Store. `settings.companies[]` ist die einzige persistierte Wahrheit für profilgebundene Einstellungen; die Produktoberfläche erzeugt und verwendet in dieser Phase weiterhin ausschließlich Profil 1. Geschäftsbereiche, Leistungsorte, Belege, Gutscheine, Rezepte und Behandlungsdatensätze referenzieren ihr Profil direkt. Kunden bleiben gemeinsam. Der strukturelle IndexedDB-Versionswechsel und die nachfolgende fachliche Sieben-Store-Migration sind technisch getrennte Transaktionen; bleibt der Prozess dazwischen stehen, erkennt jeder folgende Start den alten Settingsvertrag und wiederholt die vollständige, atomare Fachmigration. Ein inkonsistenter Bestand wird nicht teilweise geschrieben.

MULTI-COMPANY-003 behält Schema 9 bei und macht `companies[].receiptSettings.numbering` zur kanonischen Nummernkreisquelle. Je Profil, fachlichem Abschlussjahr und Typ werden getrennte nächste Sequenzen geführt. Neue, ausdrücklich konfigurierte Profile verwenden `<KÜRZEL>-JJJJ-NNNNNN`, `<KÜRZEL>-ST-JJJJ-NNNNNN` und `<KÜRZEL>-GS-JJJJ-NNNNNN`, jeweils standardmäßig ab `000001`. Profil 1 bleibt bis zur bewussten Kürzelwahl in MULTI-COMPANY-004 im kompatiblen bisherigen Format; kein Kürzel wird aus Namen oder Steuerdaten geraten. `yearPrefix` und `nextNumber` bleiben nur eine synchronisierte Kompatibilitätsprojektion für die bestehende Ein-Profil-Oberfläche. Nummer und Beleg werden in derselben IndexedDB-Readwrite-Transaktion gespeichert; Gutscheinverkaufsbelege teilen den normalen Profilkreis. Neue Korrekturen referenzieren ihren Ursprung primär über die stabile Receipt-ID und behalten die sichtbare Nummer zusätzlich für Anzeige und Export.

MULTI-COMPANY-004 behält Schema 9 bei. Die Unternehmensverwaltung arbeitet ausschließlich auf `companies[]`, `activeCompanyId` und den bereits profilgebundenen Bereichen und Orten. Ein neues Profil erhält opake IDs, ein bewusst eingegebenes Kürzel aus 2 bis 8 Großbuchstaben/Ziffern mit Buchstaben am Anfang, getrennte Zähler ab `000001` und eine eigene portable Lizenzreferenz ohne kopierte Autorität. Das bisherige Profil bleibt ohne erfundenes Kürzel im Legacy-Modus; eine spätere Kürzelwahl wirkt nur auf künftige Nummern. Profilwechsel sind bei Beleg-, Gutschein- oder Podologieentwürfen gesperrt. Settings, Listen und Exporte werden bis MULTI-COMPANY-005 sicher auf das aktive Profil begrenzt; Kunden bleiben gemeinsam. Unternehmensprofile werden in V1 nicht physisch gelöscht. Die Architekturentscheidung steht in [ADR-0008](docs/adr/ADR-0008-unternehmensprofilverwaltung-und-aktivkontext.md).

MULTI-COMPANY-004B behält Schema 9, Lizenzreferenzen und `licenseRuntime` unverändert. Ein Zusatzprofil kann nur in den ausdrücklich erlaubten Builds `BETA-PREVIEW-002` und `BETA-PREVIEW-003` den lokalen Testzustand `betaProductiveTest` erhalten. Der echte Status bleibt `activation_required`; die zentrale Produktivprüfung akzeptiert ausschließlich `regularProductive` oder die Kombination aus gespeichertem Testzustand und erlaubter Buildkennung. Alle anderen Builds ignorieren das Flag. Tenant-Snapshot, Backup, Restore, Diagnose, Export, PDF und Public-QR erhalten es nicht. Deaktivierung sperrt nur künftige Mutationen und verändert weder Historie noch Nummernkreise. Die temporäre Entscheidung steht in [ADR-0009](docs/adr/ADR-0009-temporaere-beta-testfreigabe-fuer-zusatzprofile.md).

Der gerätelokale `licenseRuntime`-Store ist kein Geschäftsdatenstore. Er wird unabhängig von den sieben Tenant-Stores geführt und niemals in Tenant-Snapshot, Backup, Restore, Diagnose oder Export aufgenommen. Eine portable Lizenzreferenz kann keine Geräteautorisierung rekonstruieren.

Verbindliche Regeln:

- Ein vollständig neuer produktiver Mandant startet ohne Katalogpositionen, Kunden, Belege, offene Zahlungen, Korrekturen, Gutscheine, Umsätze und Logoassets. Technische Defaults, nur bewusst importierbare Onboarding-Vorlagen und eng begrenzte historische Reparaturquellen müssen im Laufzeitmodell getrennt bleiben. Persistierte Mandantendaten haben immer Vorrang und dürfen durch eine Änderung der Erststartdefaults weder gelöscht noch zurückgesetzt werden. Inventar und Übergabegate stehen in `docs/beta-handoff.md`.
...
- Integritätsdiagnosen verwenden ausschließlich die zentrale Snapshot-Validierung als Wahrheitsquelle. Sie lesen die sieben Tenant-Stores in einer Read-only-Transaktion und dürfen nur Prüfcode, konkrete Invariante sowie die dafür notwendigen technischen IDs, Referenzen, Belegnummern, Belegarten und Zeit-/Historienreferenzen ausgeben. Kunden-, Unternehmens-, Positions-, Betrags-, Kennwort- und Behandlungsinhalte bleiben ausgeschlossen; eine Ausgabe erfolgt erst nach bewusster Nutzeraktion.
...
- Neben der automatischen Updatekarte darf eine manuelle Prüfung denselben zentralen Updatecontroller auslösen. Sie erzeugt weder einen zweiten Aktivierungspfad noch einen eigenen Reload.
- SERVICEWORKER-002 prüft bei einem Online-Start genau einmal auf einen neuen Worker und berücksichtigt bereits wartende Worker. UPDATE-001 behandelt zusätzlich einen bereits aktivierten Ersatzworker und begrenzt ausbleibende Aktivierungs- und Navigationsabschlüsse. UPDATE-001b prüft während des bewussten Aktivierungsversuchs zusätzlich den aktuellen Registration- und Controllerstand, sodass ein bereits vergangenes oder ausgebliebenes Lifecycle-Ereignis den Abschluss nicht blockiert. Alle Erfolgssignale teilen dieselbe Sperre und lösen gemeinsam höchstens einen Reload aus.
- Die ausdrücklich freigegebene automatische Aktivierung im SERVICEWORKER-002-Worker war eine einmalige Legacy-Brücke für ältere Clients ohne Update-UI. Die reale Übergangsabnahme mit 0.10.9 ist bestätigt; SERVICEWORKER-003 entfernt deshalb Konstante und Installations-Autoaktivierung in 0.10.10. Der automatisierte Release-Preflight bleibt als Regressionsschutz bestehen und verwirft jede künftige Wiedereinführung dieser Legacy-Konstante.
...
- Nach einem vollständig geprüften, freigegebenen und zu `origin/main` gepushten Release-Commit ist `./scripts/release-beta.sh` der normale Beta-Releaseweg. Er darf Version, Fachcode, Release-Dokumentation oder Commit nicht selbst ändern und automatisiert weder Web Station noch Produktion oder Geräteabnahme.
- Die maschinenlesbare Release-Freigabe in `docs/releases/<version>.md`, der unvergebene lokale und entfernte Tag sowie die unvergebene lokale und Synology-Release-ID sind verbindliche No-Clobber-Gates. Ein bestehender Tag wird niemals verschoben, gelöscht oder mit Force aktualisiert.
...
- automatisierte Tests und Qualitätsprüfungen weiter ausbauen; der reproduzierbare lokale Beta-Releaseprozess ist mit RELEASE-AUTOMATION-001 etabliert, während Web-Station-Umschaltung, Geräteabnahme und Produktion manuelle Freigabegates bleiben.
...
- das unter `docs/compliance-v1.md` verbindlich abgegrenzte V1.0-Betriebsmodell beibehalten: lokale Beleg- und Zahlungsdokumentation, keine vollständige Kassenführung, kein Kassenbuch und kein Ersatz für Kassenbericht oder Kassensturz;
- die individuelle steuerberaterliche TSE-Einordnung ausschließlich auf diesen konkreten Einsatz beziehen; bei abweichender Nutzung oder erweitertem Funktionsumfang erneut fachlich prüfen;
- eine vollständige TSE-/DSFinV-K-Implementierung nicht automatisch als V1.0-Gate behandeln, die vorhandene neutrale Vorbereitung jedoch für einen späteren eigenständigen Produktblock erhalten;
- vor V1.0 Steuerstatus einschließlich Kleinunternehmern und steuerfreien Leistungen, Rechnungsfelder, Gutscheinsteuerlogik, Teilkorrekturen sowie GoBD-/Restore-/Aufbewahrungsgrenzen in COMPLIANCE-003 bis COMPLIANCE-006 klären;
- Lizenzierung sowie reale Geräte-, Release- und Betriebsgates weiterhin unabhängig erfüllen.
...
- Die Installation als Home-Screen-App ist mit ONBOARDING-001 verständlich und jederzeit auffindbar erklärt. Offen bleibt die weitergehende Verbindung mit der Ersteinrichtung: Online-Erststart, vollständig geladene App-Shell, Wiederaufnahme der Einrichtung und erkennbare Offline-Bereitschaft sollen später einen zusammenhängenden Ablauf bilden.
