...
- SERVICEWORKER-002 prüft bei einem Online-Start genau einmal auf einen neuen Worker und berücksichtigt bereits wartende Worker. UPDATE-001 behandelt zusätzlich einen bereits aktivierten Ersatzworker, begrenzt ausbleibende Aktivierungs- und Navigationsabschlüsse und stellt sicher, dass `activated` und `controllerchange` nach der bewussten Aktion gemeinsam höchstens einen Reload auslösen.
- Die ausdrücklich freigegebene automatische Aktivierung im SERVICEWORKER-002-Worker ist eine einmalige Legacy-Brücke für ältere Clients ohne Update-UI. Sie verwendet weder `clients.claim()` noch einen automatischen Reload. Da der reale Übergang der bereits ausgelieferten Altclients noch nicht bestätigt ist, bleibt sie in 0.10.8 fachlich unverändert erhalten; nach bestätigter Übergangsabnahme muss sie im unmittelbar folgenden Worker/Release entfernt werden.
